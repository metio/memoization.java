...
Popular JCache implementations are link:http://www.ehcache.org/[Ehcache], link:http://commons.apache.org/proper/commons-jcs/[Commons JCS], link:https://hazelcast.org/[Hazelcast], link:http://infinispan.org/[Infinispan] and link:http://www.alachisoft.com/tayzgrid/[TayzGrid].
Use link:https://github.com/jhalterman/expiringmap[ExpiringMap] and other `ConcurrentMap` implementation as an alternative to the default `ConcurrentHashMap` in the `MapMemoize` class.
