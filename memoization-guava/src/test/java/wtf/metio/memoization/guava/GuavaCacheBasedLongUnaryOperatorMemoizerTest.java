...
class GuavaCacheBasedLongUnaryOperatorMemoizerTest {
    ...
    void shouldAcceptLoadingCache() {
        ...
        final Cache<Long, Long> cache = CacheBuilder.newBuilder().build();

        // when
        final var memoizer = new GuavaCacheBasedLongUnaryOperatorMemoizer<>(cache, keyFunction, function);
    ...
    void shouldTransformInput() {
        ...
        // then
        Assertions.assertEquals(123, memoizer.applyAsLong(789));
    }
    ...
    void shouldWrapExecutionExceptionInMemoizationException() throws ExecutionException {
        // given
        final LongFunction<Long> keyFunction = Long::valueOf;
        final Cache<Long, Long> cache = Mockito.mock(Cache.class);
        given(cache.get(any(), any())).willThrow(ExecutionException.class);

        // when
        final var memoizer = new GuavaCacheBasedLongUnaryOperatorMemoizer<>(cache, keyFunction, null);
