...
public class JCacheBasedToLongFunctionMemoizerTest {
    ...
    */
    ...
    public void shouldMemoizeBiFunction() {
        // given
        final ToLongFunction<String> function = first -> 123;
        ...
        try (final Cache<String, Long> cache = JCacheMemoize.createCache(ToDoubleBiFunction.class)) {
            // when
            final JCacheBasedToLongFunctionMemoizer<String, String> loader = new JCacheBasedToLongFunctionMemoizer<>(
                    cache, keyfunction, function);

            // then
            Assert.assertEquals("Memoized value does not match expectation", 123, loader.applyAsLong("first"));
        }
    }
