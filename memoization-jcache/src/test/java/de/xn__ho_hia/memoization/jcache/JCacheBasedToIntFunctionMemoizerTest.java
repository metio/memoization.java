...
public class JCacheBasedToIntFunctionMemoizerTest {
    ...
    */
    ...
    public void shouldMemoizeBiFunction() {
        // given
        final ToIntFunction<String> function = first -> 123;
        ...
        try (final Cache<String, Integer> cache = JCacheMemoize.createCache(ToDoubleBiFunction.class)) {
            // when
            final JCacheBasedToIntFunctionMemoizer<String, String> loader = new JCacheBasedToIntFunctionMemoizer<>(
                    cache, keyfunction, function);

            // then
            Assert.assertEquals("Memoized value does not match expectation", 123, loader.applyAsInt("first"));
        }
    }
