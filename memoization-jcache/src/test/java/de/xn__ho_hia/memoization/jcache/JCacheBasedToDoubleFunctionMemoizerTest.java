...
public class JCacheBasedToDoubleFunctionMemoizerTest {
    ...
    */
    ...
    public void shouldMemoizeBiFunction() {
        // given
        final ToDoubleFunction<String> function = first -> 123;
        ...
        try (final Cache<String, Double> cache = JCacheMemoize.createCache(ToDoubleBiFunction.class)) {
            // when
            final JCacheBasedToDoubleFunctionMemoizer<String, String> loader = new JCacheBasedToDoubleFunctionMemoizer<>(
                    cache, keyfunction, function);

            // then
            Assert.assertEquals("Memoized value does not match expectation", 123D, loader.applyAsDouble("first"), 0.0D);
        }
    }
